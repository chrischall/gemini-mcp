 ...
 *
 * The read tools in this server hand back Gemini's payload close to
 ...
 * honestly say which of Gemini's fields matter and which are noise.
 *
 ...
 */
export const GM_VIEWS = ['compact', 'full'] as const;

const NOTE =
  'compact strips image/avatar URLs from the response; "full" returns Gemini\'s payload untouched. ' +
  'No field projection: this server has no verified record of which Gemini fields matter, and inventing ' +
  'one would risk dropping a field a caller needs.';

/** The `view` parameter every read tool in this server takes. */
export const viewArg = (): ReturnType<typeof viewParam> => viewParam(GM_VIEWS, { note: NOTE });
...
export function viewResponse(view: string | undefined, data: unknown): ReturnType<typeof minifiedResult> {
  const rung: View = resolveView(view, GM_VIEWS);
